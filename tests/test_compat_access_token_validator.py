...
def test_access_token_validator_passes_with_valid_token(
    access_token: str,
...
def test_access_token_validator_passes_with_jwt_typ_header_value() -> None:
    response = httpx.post(
        "http://keycloak:8002/realms/rfc9068/protocol/openid-connect/token",
        data={
            "client_id": "test-client-with-jwt-type-header",
            "client_secret": "nbbliMNNsrTHDNpMVyVx9AR1CHAKJey1",
            "grant_type": "client_credentials",
        },
    )
...
def test_raises_when_value_is_list_and_does_not_contain_expected_audience() -> None:
    response = httpx.post(
        "http://keycloak:8002/realms/rfc9068/protocol/openid-connect/token",
        data={
            "client_id": "test-client-with-multiple-incorrect-audiences",
            "client_secret": "Av1Sm48jUykZgU0nKztE72ZI8SmsIpjW",
            "grant_type": "client_credentials",
        },
    )
